...
buildMessageListMenu(
  document.querySelector("#deleteListsForm"),
  "lists-delete-id"
);

// stoffe: Build and insert the List to Display picker on the lists page.
buildMessageListSelector(
  document.querySelector("#shoppingListHeading"),
  "message-list-show",
...
if (removeListForm !== undefined && removeListForm !== null) {
  removeListForm.addEventListener("submit", (event) => {
    ...
    if (getListIdExists(listIdSelect.value)) {
      if (
        confirm(
          "Are you sure you want to remove this list? All associated messages are deleted as well."
        )
      ) {
...
function buildMessageListSelector(
  containerElem,
  elementId,
  listSelectedCallback
) {
